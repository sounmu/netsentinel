...
The dashboard **and** the API share **http://localhost:3000** in the production-style container. Open `/setup` for the first-admin flow; Google OAuth is optional. In local dev, Next runs on `http://localhost:3001`; if OAuth is enabled, its callback still points directly at the Rust API (`http://localhost:3000/api/auth/oauth/google/callback`) and the API redirects back to the frontend after issuing cookies. `docs/AFTER_INSTALL.md` walks through the full 10-minute path (admin → first host → first agent). Use `./scripts/doctor.sh` if anything above fails.
