#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# NetSentinel host monitor — one-liner installer / updater
#
...
#
# The host monitor is pull-scraped by the hub. New installs exchange a short-lived
...
print_help() {
  cat <<'HLP'
NetSentinel host installer / updater

Usage:
  sudo bash install-host.sh [options]

Options:
  --server-url URL      NetSentinel hub URL               env: NS_SERVER_URL
  --enroll-token VALUE  one-time enrollment token          env: NS_ENROLL_TOKEN
  --jwt-secret VALUE    legacy/manual auth secret          env: NS_JWT_SECRET
  --network MODE        lan or tailscale [lan]             env: NS_NETWORK_MODE
  --port N              port the host monitor listens on [9101] env: NS_AGENT_PORT
...
Recommended path from the NetSentinel UI:
  curl -fsSL .../install-host.sh | sudo -E bash -s -- \
...
if [[ $EUID -ne 0 ]]; then
  ...
  echo "    Example: curl -fsSL .../install-host.sh | sudo bash -s -- --server-url URL --enroll-token TOKEN" >&2
  exit 1
fi
...
if [[ -n "$ENROLL_TOKEN" || -n "$SERVER_URL" ]]; then
  ...
  echo "    Open NetSentinel → Hosts → Add Host and copy the generated command." >&2
  echo "    Legacy/manual installs may pass --jwt-secret directly." >&2
  exit 1
fi

if [[ -n "$JWT_SECRET" && ${#JWT_SECRET} -lt 32 ]]; then
  echo "❌ Host auth secret is only ${#JWT_SECRET} chars; it must be ≥ 32." >&2
  exit 1
fi
...
This installer was asked to build the host monitor from source via
...
# ── write host monitor config ───────────────────────────────────────
cat > "${CONFIG_FILE}" <<EOF
# Managed by scripts/install-host.sh — re-run with different flags to replace.
...
case "$os" in
  Linux)
    if ! command -v systemctl >/dev/null 2>&1; then
      ...
      cat > "$unit_path" <<EOF
[Unit]
Description=NetSentinel host monitor (pull-scraped monitoring service)
...
Restart=on-failure
RestartSec=5
# Hardening: the monitor only needs to listen on a TCP port and read
# its own env file.
...
if [[ -n "$ENROLL_TOKEN" ]]; then
  ...
  next_step="Legacy/manual install: open Hosts → + Add Host on the hub and enter host address ${HOST_KEY}."
fi

cat <<EOM

─────────────────────────────────────────────────────────────────────
✅ Host monitor installed and running.
