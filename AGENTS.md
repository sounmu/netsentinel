...
- **Where used**: SSE payloads, DB columns, REST API path (`/api/metrics/{host_key}`), frontend route (`/host/[host_key]`), in-memory caches (`last_known_status` keyed by `host_key`). `HostStatusPayload` also carries `scrape_interval_secs` so the web status-aging logic matches the server scheduler.
