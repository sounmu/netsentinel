...
- **Alerts page revamp — Phases 1-3** (Beszel-inspired UX + DESIGN.md M3 compliance).
  - **Phase 1 (web only)** — introduce the M3 color + typography tokens in `globals.css` and replace every inline-styled alerts page element with a `.alerts-*` utility-class BEM namespace. No behaviour change, just a token-compliant surface (surface-container-low cards with outline-variant borders, corner-full buttons in filled/tonal/outlined/danger variants, outlined text fields with the DESIGN.md §8.6 focus-padding trick, inverse-surface feedback chips, `aria-live` status regions).
  - **Phase 2 (web only)** — restructure the page into a four-tab IA (Active / Rules / History / Channels) synced to `?tab=` via `useSearchParams` inside a Suspense boundary (Next.js 16 requires it). New components under `app/alerts/components/`: AlertsTabs, AlertsSummaryBar, ActiveAlertsPanel, RulesPanel, RulesMatrix, BulkApplyBar, HistoryPanel, ChannelsPanel, MetricRuleCard (range slider + natural-language preview), shared helpers. Relative-time ticks use `useSyncExternalStore` to satisfy React 19's new purity rules.
  - **Phase 3 (server + web)** — migration `017_expand_alert_metric_types.sql` widens `alert_configs.metric_type` to include `load / network / temperature / gpu` and adds an optional `sub_key` column scoped per sensor / interface / GPU; unique index becomes `(host_key, metric_type, sub_key) NULLS NOT DISTINCT`. `MetricType` enum grows the four new variants, `UpsertAlertRequest` / `AlertConfigRow` expose `sub_key`, and `validate_alert_request` applies per-metric threshold ranges (CPU/Memory/Disk/GPU 0-100 %, Load 0-64, Network 0-10 Gbps, Temperature −20-120 °C). Two new admin endpoints: `POST /api/alert-configs/bulk` (single transaction, cap 500 hosts) and `GET /api/alerts/active` (`DISTINCT ON` over 14 d of `alert_history`, keeping only latest-is-firing events). `GET /api/alert-history` gains `type`/`from`/`to` filters and returns `{rows, total}` for cursor-free pagination. Scraper-side evaluation for load/network/temperature/gpu is intentionally deferred — rules store and surface today, evaluate in a later milestone.
